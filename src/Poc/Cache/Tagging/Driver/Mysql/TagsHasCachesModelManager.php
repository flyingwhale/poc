...
class TagsHasCachesModelManager extends ModelManager
{
    ...
    public function __construct ($dbHandler)
    {
        ...
        $this->setQuery('create',
                'CREATE TABLE IF NOT EXISTS `tags_has_caches` (
    ...
    public function find ($tag_id, $cache_id)
    {
        ...
        $params = array(':tag_id' => $tag_id, ':cache_id' => $cache_id);
    ...
    public function delete (\POC\Cache\Tagging\Driver\Mysql\TagCache $model)
    {
        ...
        $query = sprintf(
                'DELETE FROM %s WHERE cache_id = :cache_id AND tag_id = :tag_id',
                $this->getTableName());

        $params = array(':cache_id' => $model->cache_id, ':tag_id' => $model->tag_id);
        $sth = $dbh->prepare($query);
    ...
    public function deleteByHashTags ($hash, $tags)
    {
        ...
        $query = sprintf(
                'DELETE ths FROM tags_has_caches ths
            LEFT JOIN tags t  ON (t.id = ths.tag_id)
            LEFT JOIN caches c  ON (c.id = ths.cache_id)
            WHERE c.hash = :hash AND t.tag IN (%s)', $tagsString);
